...
## GreenDICE main specification (START)
        ...
        set_param!(GreenDICE, :green_naturalcapital, :invparam, 0.5169)
    ...
    F = 1/(1 + 0.050642536*0.82^2)
    D = (1 - F)
    B = ((D/0.000001)-1)/0.00123
    B05 = ((D/0.01)-1)/(0.00123^0.5)
    b = log((D / 0.01)-1)/log(76)
    
    B = (D / (0.1*47) - 1)/(0.00123^0.1)

    
    b = log(D / (0.01*47) - 1)/log(0.00123)
    log((0.032930684191360915/0.01)-1)/log(0.00000000123)
