...
#defining functions (start)
    ...
    function eval_dice_invAsset(x)
        m = x[1:60]
        s = x[61:120]
        inv = x[121:180]
        set_param!(GreenDICE,:emissions,:MIU,m)
        set_param!(GreenDICE,:neteconomy,:S,s)
        set_param!(GreenDICE,:neteconomy,:AssetNCfrac,inv)
        run(GreenDICE)
        
        return -GreenDICE[:welfare, :UTILITY]
    end

    function eval_dice_inv2(x)
        m = x[1:60]
        s = x[61:120]
        inv = x[121:180]
        inv2 = x[181:240]
        ...
        set_param!(GreenDICE,:neteconomy,:S,s)
        set_param!(GreenDICE,:neteconomy,:AssetNCfrac,inv)
        set_param!(GreenDICE,:neteconomy,:invNCfrac,inv2)
...
## GreenDICE main specification (START)
    ...
    while mc < mc_max
        resinv = bboptimize(eval_dice_inv;SearchRange=(0.,1.0), NumDimensions=180, Method=:adaptive_de_rand_1_bin_radiuslimited,MaxSteps=10_000)
    ...
    while mc < mc_max
        priceN = pricesNC()
                    ...
                    run(GreenDICE)
                    # GreenDICE[:green_naturalcapital,:benefitsNC][35:60]
                    # GreenDICE[:green_naturalcapital,:invNC][35:60]
                    # GreenDICE[:neteconomy,:AssetNCfrac][35:60]
                    # priceN.inv[35:60]
                    resinv = bboptimize(eval_dice_invAsset;SearchRange=(0.,1.0), NumDimensions=180, Method=:adaptive_de_rand_1_bin_radiuslimited,MaxSteps=100_000)
                    best_candidate(resinv) # optimal vector of miu emissions trajectories
                    set_param!(GreenDICE,:emissions,:MIU,best_candidate(resinv)[1:60])
                    set_param!(GreenDICE,:neteconomy,:S,best_candidate(resinv)[61:120])
                    set_param!(GreenDICE,:neteconomy,:AssetNCfrac,best_candidate(resinv)[121:180])
                    run(GreenDICE)
                    results = foo()
    ...
    CSV.write("C:/Users/bastien/Documents/GitHub/GreenDICE/Results/investments_iterations.csv",Results_investment_iterations)
    set_param!(GreenDICE,:neteconomy,:invNCfrac,fill(0.,60))
    set_param!(GreenDICE,:neteconomy,:AssetNCfrac,fill(0.,60))
    run(GreenDICE)
    mc_max = 100
    ...
    while mc < mc_max
        priceN = pricesNC()
                    set_param!(GreenDICE,:neteconomy,:priceNC,priceN.inv)
                    run(GreenDICE)
                    resinv = bboptimize(eval_dice_inv2;SearchRange=(0.,1.0), NumDimensions=240, Method=:adaptive_de_rand_1_bin_radiuslimited,MaxSteps=100_000)
                    best_candidate(resinv) # optimal vector of miu emissions trajectories
                    set_param!(GreenDICE,:emissions,:MIU,best_candidate(resinv)[1:60])
                    set_param!(GreenDICE,:neteconomy,:S,best_candidate(resinv)[61:120])
                    set_param!(GreenDICE,:neteconomy,:AssetNCfrac,best_candidate(resinv)[121:180])
                    set_param!(GreenDICE,:neteconomy,:invNCfrac,best_candidate(resinv)[181:240])
    ...
    CSV.write("C:/Users/bastien/Documents/GitHub/GreenDICE/Results/2investments_iterations.csv",Results_investment_iterations)
